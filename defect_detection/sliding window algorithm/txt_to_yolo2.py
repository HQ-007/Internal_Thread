...
# 裁剪后label文件位置
labels_path = r'/home/huangqing/YOLOv5_内螺纹/YOLOv5small/small_0.3/labels_yolo'
...
for txt_name in txt_names:
    ...
    # 处理不同宽高的图片，仅限于 分成 1*10 的， 混合度 50%
    child_height = int(img_height[0]) // 1
    child_width = int(img_width[0]) // 10
    mix_row_width = int(child_width * 0.5 * 2)
    mix_row_height = child_height
    mix_col_width = child_width
    mix_col_height = int(child_height * 0.5 * 2)
    row_width_step = int(child_width * 0.5)
    # col_height_step = int(child_height * 0.5)
